...
@Component
public class UpdatePriceDetailsCommand extends ACommand<CommandContext> {
  ...
  public Boolean execute(Boolean ignore, CommandContext commandContext) {
    ...
    Price newPrice = (Price)commandContext.getRequest().get("price");
    price.setValue(newPrice.getValue());
    price.setCurrencyCode(newPrice.getCurrencyCode());
  ...
  public Boolean postProcess(Boolean result, CommandContext context) {
    this.logger.info("step=UPDATE_PRICE_DETAILS, status=SUCCESS");
    return CommandChain.CONTINUE;
  }

  public Boolean handleException(Exception e, Boolean result, CommandContext context) throws Exception {
    this.logger.error("step=UPDATE_PRICE_DETAILS, status=FAIL");
    throw e;
  }
}
